...
def _read_packaged_asset_text(*parts: str) -> str:
    # Assets are shipped as package data (see pyproject.toml) under gitig/assets/.
    try:
        return resources.files("gitig.assets").joinpath(*parts).read_text("utf8")
    except Exception as exc:
        raise FileNotFoundError("/".join(parts)) from exc


def read_asset(*parts: str) -> str:
    key = "/".join(parts)
    try:
        return _read_packaged_asset_text(*parts)
